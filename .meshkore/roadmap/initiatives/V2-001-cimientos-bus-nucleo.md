...
- 2026-07-09 · T37 — esqueleto de `nucleo/` (cerebro v2): árbol de stubs con docstrings + firmas que fijan el contrato — `flash/{router,fast_client,frontend,procs,escalate}`, `loop`, `dispatch`, `memory_agent`, `agentes/{base(CodeAgent ABC),claude_code,codex}`. Reglas duras codificadas en las firmas (modelo POR INVOCACIÓN, no-razonador, deny-tools para input no confiable). `WIRED_TO_VOICE=False` — nada cableado a la voz. 6 tests verdes (`tests/agent_headless/unit/test_skeleton.py`: imports + contrato + stubs levantan NotImplementedError).
