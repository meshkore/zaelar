...
| `show_widget` | **2026-07-17** · SHOW/open/play a widget on the canvas, incl. GAMES: "abre el reloj", "muéstrame X", "juega al snake". A **first-class tool** (sibling of `play_video`) because a text tag `[[show]]` loses to a function-calling tool when the word collides ("jugar"≈play → play_music/video hijacked it; prose fixes were 2-3/5 flaky, tool is 5/5). Provider+probe converge on `[[show:id]]`; id resolved exact-or-fuzzy via `runtime.identify` (never invented). NOT play_music/play_video (playing≠showing), NOT widget_data (data). | → provider `_tag_emit("show", id)` | when widgets exist |
