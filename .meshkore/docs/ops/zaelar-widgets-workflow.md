...
| **Cualquier cadena VISIBLE de un widget de serie** (rótulo, título, botón, estado vacío, `placeholder`, `title`) | `tt("clave", params, "<el literal>")` en el `widget.js` + la fila en `i18n/bundles/en.json` **y** `es.json` · bump de `MANIFEST_VERSION` si cambia el CONJUNTO de claves · el gate del nodo 4.11 lo exige y falla el build (V2-694) |
| **El NOMBRE de un widget o de una superficie** | `widgets.<id>.name` / `surfaces.<id>.name` en los dos bundles — NO el `manifest.json`, que se queda como fallback y como alias para que la voz siga respondiendo a la palabra de fábrica · `widgets/registry.py::display_name` es el único lector (V2-694) |
