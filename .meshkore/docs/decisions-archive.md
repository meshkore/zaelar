# Decisiones clave — archive (engine/CLAUDE.md)

Verbatim decision entries moved out of `engine/CLAUDE.md` by the V2-601 T-18 compaction (2026-09-06),
in their original order. Nothing here was edited or summarized on the way out. The living log and the
one-line index that replaces these entries stay in `engine/CLAUDE.md`; each entry's initiative under
`.meshkore/roadmap/initiatives/` remains the dense source. New archive passes append below with a dated
`## Moved on YYYY-MM-DD` marker.
...
## Moved on 2026-09-14 (second pass)
...
- **The turn asks Jev ONCE, and the voice path stopped blocking on it (V2-726 F1+F2, 2026-09-20)**: measured, Jev's cost is the ROUND TRIP and not the questions — 1 question 800 ms, 4 heterogeneous 708-826 ms, 100 candidates 1041 ms — while the fast model's TTFT alone is 1.9 s. The engine was making TWO trips per turn for two questions and would have made a third for the route. Now `nucleo/flash/turn_brief.py` fires ONE call at turn start carrying every question the turn reads AFTER the model answers: the canvas verb, the escalate pair, and which declared action of the OPEN widgets an order is aimed at. **Why only post-model questions**, and this is the half the first draft of the plan had backwards: over 87 real turns the prompt is assembled 3 ms after the turn is admitted and the tool catalog is settled at 381 ms, while a Jev verdict lands at 785 ms — so nothing decided BEFORE the model can be served by a brief fired at t0 without making the model wait for it, and what is decided after has 2-4 s of head start. A pre-model question belongs on the interim transcript, which is a different and still unmeasured design. **F2, and it is the part that was actually dangerous**: `judge_escalation` and `repair_action` called `choose_sync` — a blocking `urlopen` — from inside the voice provider's `async def _run_inner`, freezing for up to 900 ms the event loop STT, TTS and barge-in share, and the escalate gate is the one that times out most (28% of its calls). Both now READ the brief; the blocking functions stay for the probe/text channel, which has no loop to freeze, and a ratchet pins that the provider names neither. **Nobody ever waits**: readers `peek`, and a brief in flight, failed, unsure or `ZAELAR_JEV=0` all read as today's path bit-for-bit — which matters because 56 of 87 real directed turns died to barge-in before they finished, so a reader that blocked would be paying for turns that no longer exist. The canvas verb was folded in rather than retired: it changed ZERO outcomes in 159 real calls (the model emits its own show/close tags and the grammar vetoes the spares), but inside a brief a backstop costs nothing — what was retired is its second round trip. Bounds live with the only wire call: `MAX_QUESTIONS`/`MAX_STATE_CHARS` refuse an oversized brief before the socket, because a caller that enumerates a database would turn one cheap classifier into a slow expensive one and fail as a truncated verdict rather than an error. Node 3.64, five disarms red — one of which had to be rewritten first: the owner check on a repair passed disarmed because the test used two widgets with no shared action name, so the declared-actions filter caught it anyway; `musica` and `youtube` share eight, and that is the case that makes the guard load-bearing.
...
- **The agent gets its OWN filesystem, and the torrent client becomes a SYSTEM tool (V2-638, 2026-09-09)** — texto íntegro en `decisions-archive.md`
