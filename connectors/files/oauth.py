...
def client_id(provider_id: str) -> str:
    return _cred(f"FILES_{provider_id.upper()}_CLIENT_ID") or _shipped_google(provider_id, secret=False)


def client_secret(provider_id: str) -> str:
    return _cred(f"FILES_{provider_id.upper()}_CLIENT_SECRET") or _shipped_google(provider_id, secret=True)
