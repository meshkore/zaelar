...
const OVERALL = { ok: "All good", warn: "Attention", error: "There's a problem", unknown: "Checking…" };
...
export function StatusPanel() {
  ...
  const panel = h("div", { class: () => "statuspanel" + (store.statusOpen() ? " open" : ""), ref: (el) => (panelEl = el) },
    h("div", { class: "cw-head" },
      h("button", { class: "cw-grip", ref: (el) => (gripEl = el), title: "Move" }, raw(DRAG_SVG)),
      h("span", { class: "cw-title" }, raw(ACTIVITY_ICON), () => "Status · " + (OVERALL[store.status().overall] || "")),
      h("button", { class: "cw-x hb-icbtn", title: "Refresh", onClick: refreshStatus }, raw(REFRESH_ICON)),
      h("button", { class: "cw-x hb-icbtn", title: "Close", onClick: () => store.setStatusOpen(false) }, raw(CLOSE_ICON)),
    ),
    h("div", { class: "st-list" },
      () => {
        ...
        if (!items.length) return h("div", { class: "cron-empty" }, "Checking status…");
        const core = items.filter((it) => (it.group || "core") === "core");
        const extra = items.filter((it) => it.group === "extra");
        const out = [sec("Core · what you start"), ...core.map((it) => row(it))];
        if (extra.length) out.push(sec("Secondary", "st-sec-2"), ...extra.map((it) => row(it, true)));
        ...
        if (alerts.length) {
          out.push(sec("APIs · balance", "st-sec-2"),
            ...alerts.map((a) => row({ key: a.key, label: a.key, state: a.state, detail: a.detail || "check balance" }, true)));
        }
        return out;
      },
    ),
