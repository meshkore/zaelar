...
const ZONES = [
  { key: "state", frac: 0.10, title: "ESTADO", sub: "conciencia de sí mismo y su entorno", accent: "var(--hb-neutral)" },
  { key: "short", frac: 0.20, title: "CORTO PLAZO", sub: "memoria reciente", accent: "var(--hb-accent2)" },
  { key: "long",  frac: 0.70, title: "LARGO PLAZO", sub: "hechos durables · preferencias · tareas", accent: "var(--hb-accent)" },
];
...
function stateItems(s) {
  ...
  return [
    { label: "misión", val: s.mission },
    { label: "operador", val: s.operator_name },
    { label: "trato", val: s.treatment },
    { label: "ubicación", val: s.location },
    { label: "idioma", val: s.language },
    { label: "asistente", val: s.assistant_name },
    { label: "widgets abiertos", val: arr(s.open_widgets) },
    { label: "tareas en marcha", val: arr(s.activity) },
    { label: "recientes", val: arr(s.recent) },
    { label: "temas", val: arr(s.topics) },
  ];
}
...
export function MemoryMap() {
  ...
  function memCard(m, x, y, w) {
    ...
    el.title = `#${m.id} · ${m.kind} · ${m.level}\n${m.text || ""}\n\nscore ${num(m.importance)} · peso ${num(m.weight)} · accesos ${m.access_count || 0}` +
      (m.pinned ? " · 📌" : "") + `\ncreado ${fmtDate(m.created)} · visto ${fmtDate(m.last_access)}` +
  ...
  function stateCard(it, x, y, w) {
    ...
    el.title = it.label + ": " + (empty ? "(vacío)" : it.val);
  ...
  function renderSlots(data) {
    ...
    for (const z of ZONES) {
      ...
      if (n === 0) {
        ...
        none.textContent = z.key === "state" ? "(estado vacío — se poblará al conocerte)" : "(sin recuerdos en esta capa)";
  ...
  function conceptNode(n, cx, cy, r) {
    ...
    el.title = `${n.label} — ${n.count} dato(s)`;
  ...
  function conceptPanel(meta, graph, x, y, w) {
    ...
    if (!nodes.length) {
      ...
      none.textContent = "(sin conceptos en esta capa todavía)";
      nodesEl.appendChild(none);
      return y + panelH;
    }
    ...
    for (const l of links) {
      ...
      ti.textContent = `${l.a} ↔ ${l.b} · ${l.weight} dato(s) en común`;
      line.appendChild(ti);
  ...
  function renderConcepts(data) {
    ...
    let y = PAD;
    y = conceptPanel({ key: "short", title: "CORTO PLAZO", sub: "mapa conceptual de la memoria reciente", accent: "var(--hb-accent2)" }, cg.short, PAD, y, panelW);
    y += COL_GAP;
    y = conceptPanel({ key: "long", title: "LARGO PLAZO", sub: "mapa conceptual de los hechos durables", accent: "var(--hb-accent)" }, cg.long, PAD, y, panelW);
    worldH = y + PAD;
    edgesSvg.setAttribute("width", worldW);
    edgesSvg.setAttribute("height", worldH);
    edgesSvg.setAttribute("viewBox", `0 0 ${worldW} ${worldH}`);
  ...
  function render(data) {
    ...
    if (countsEl) {
      ...
      countsEl.textContent = mode === "concepts"
        ? `${cg.short?.nodes?.length || 0} conceptos corto · ${cg.long?.nodes?.length || 0} conceptos largo`
        : `${c.short || 0} corto · ${c.long || 0} largo · ${cn} conceptos`;
    }
  }
  ...
  return h("div", { class: () => "memmap" + (store.memOpen() ? " open" : "") },
    h("div", { class: "mm-head" },
      h("span", { class: "mm-brand" }, raw(BRAIN), h("b", {}, "Mapa de la memoria")),
      h("div", { class: "mm-modes" },
        h("button", { class: "mm-mode on", ref: (el) => (slotsBtn = el), title: "Ver la memoria tal como se guarda (estado · corto · largo)", onClick: () => setMode("slots") }, "Slots"),
        h("button", { class: "mm-mode", ref: (el) => (conceptsBtn = el), title: "Ver el mapa conceptual: cómo se organiza y relaciona la información", onClick: () => setMode("concepts") }, "Conceptos"),
      ),
      h("span", { class: "mm-counts", ref: (el) => (countsEl = el) }, ""),
      h("div", { class: "mm-tools" },
        h("button", { class: "mm-btn hb-icbtn", title: "Alejar", onClick: () => zoomBy(1 / 1.2) }, "−"),
        h("button", { class: "mm-btn hb-icbtn", title: "Acercar", onClick: () => zoomBy(1.2) }, "+"),
        h("button", { class: "mm-btn hb-icbtn", title: "Ajustar a la vista", onClick: () => fitView() }, raw(FIT_ICON)),
        h("button", { class: "mm-btn hb-icbtn", title: "Refrescar", onClick: () => load(false) }, raw(REFRESH_ICON)),
        h("button", { class: "mm-btn mm-close hb-icbtn", title: "Cerrar (Esc)", onClick: close }, raw(CLOSE_ICON)),
      ),
    ),
    h("div", { class: "mm-viewport", ref: (el) => (viewportEl = el), onWheel, onPointerdown: onPointerDown },
      h("div", { class: "mm-hint" }, "rueda = zoom · arrastra = mover · Slots = contenido · Conceptos = organización"),
      h("div", { class: "mm-empty", ref: (el) => (emptyEl = el) }, "La memoria está vacía todavía. Habla con zaelar y verás cómo se puebla aquí en vivo."),
