...
#
#   ./.venv/bin/python -m tests.agent_headless.harness.run              # all personas + judge
#   ./.venv/bin/python -m tests.agent_headless.harness.run skeptic 8    # one persona, 8 turns
#
import json
import os
import sys
...
RUNS_DIR = os.path.join(ZAELAR, "tests", "runs", "harness")
...
from .user_sim import UserSim  # noqa: E402
...
def main():
    ...
    runid = _runid()
    outdir = os.path.join(RUNS_DIR, runid)
