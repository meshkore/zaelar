...
def test_el_arnes_caido_y_el_juez_mudo_son_distintos():
    a = _ronda(run={"crashed": True, "transcript": [], "mechanism_report": {}})
    S._state(3, a)
    b = _ronda()
    S._state(None, b)
    assert a["_infra_reason"] != b["_infra_reason"]
    assert "arnés" in a["_infra_reason"] and "juez" in b["_infra_reason"]
