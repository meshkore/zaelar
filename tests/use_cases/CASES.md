...
**Status: mostly backlog, 20 promoted.** Every case below is registered and browsable
...
6. **Multi-agent coordination over email** — the flagship differentiator. Buildable once contact
   resolution and the agent-message tag exist.
7. **Multi-agent coordination over WhatsApp/Telegram** — same shape as tier 6, still **BLOCKED**
   (see below) but no longer for the reason this list gave until 2026-08-31. Sending works on all
   three connectors since V2-521; what is missing is one step earlier in the sentence — *who* «the
   agent of Pedro» is. Kept in the catalog rather than silently dropped, so the gap stays visible.
...
Multi-agent cases need three things. **One of the three got built on 2026-08-31** — worth reading
carefully, because the list said the opposite for months and the difference changes what to work on:

1. **Contact resolution** ("Pedro" → a real handle on some channel). Written, not built:
   `.meshkore/roadmap/initiatives/V2-523-the-contacts-agenda.md` (supersedes the older V2-052
   sketch). One identity ↔ many handles — email, telegram, whatsapp, and their cluster AGENT —
   over a single set that also holds entities and places, with nesting and free groupers. This is
   now the FIRST blocker of every tier 6/7 case, and the only one for most of them.
2. **Write permissions per platform**, so an *unprompted* outbound message has a rule behind it:
   `.meshkore/roadmap/initiatives/V2-522-write-permissions-per-platform.md`, also written and not
   built. Note what it does NOT block: a reply the operator dictated is already covered by the
   confirm gate, and by the operator's own principle that **an explicit order IS the permission**.
...
~~**WhatsApp/Telegram send capability**~~ — *no longer a blocker.* Until 2026-08-31 both connectors
were read-only, which is why tier 6 (email) was described as reachable sooner than tier 7. V2-521
wired both to the `msg.reply` seam that had existed unused since V2-051: `connectors/whatsapp/
service.py::_drain_replies` sends over the bridge's `POST /send`, `connectors/telegram/service.py`
over Telethon, both threading the reply to the original message. So the *channel* half of tiers 6
and 7 is now symmetric, and the two tiers are the same distance away.
