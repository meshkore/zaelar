"""test_credentials.py — credential store writer (V2-040). Verifies: persistence + chmod 600, quoting of
values with spaces, redacted view (presence only, NEVER the value), deletion with an empty value, and rejection of
invalid names. Run: .venv/bin/pytest tests/infrastructure/unit/config/test_credentials.py
"""
import os
...
def test_set_persists_chmod_600_and_hot_env(store):
    ...
    assert oct(store.STORE.stat().st_mode & 0o777) == "0o600"     # owner only
    assert os.getenv("AIMLAPI_KEY") == "sk-secret-123"            # applied immediately
...
def test_status_redacts_value(store):
    ...
    assert "sk-supersecret" not in str(st)                        # the value NEVER appears
...
def test_value_with_spaces_is_quoted(store):
    ...
    assert store.get("FOO_TOKEN") == "abc def"                    # and is read without quotes
