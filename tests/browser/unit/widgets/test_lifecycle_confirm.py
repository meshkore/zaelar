"""Widget lifecycle and deletion-confirmation tests (V2-017)."""
import asyncio
import json
import os
...
def test_confirm_request_pending_resolve():
    assert confirm.request("delete", "Meteo") == "meteo"           # normalizes to lowercase
    assert "meteo" in confirm.pending()
    p = confirm.resolve("meteo", ok=True)
    assert p and p["widget_id"] == "meteo" and p["action"] == "delete"
    assert "meteo" not in confirm.pending()                        # resolved → no longer pending
    assert confirm.resolve("meteo", ok=True) is None               # there is nothing to resolve twice


def test_confirm_resolve_without_id_picks_the_only_one():
    confirm.request("delete", "agenda")
    p = confirm.resolve("", ok=True)                               # voice does not always name the widget
    assert p and p["widget_id"] == "agenda"


def test_confirm_carries_data_op(monkeypatch):
    """V2-025: an irreversible DATA action opens a confirmation that CARRIES the mutation — when resolved, the
    caller dispatches it through apply_action (it never escalates to code)."""
    ...
    assert "drop_project" in line                                  # the brain state names the action
    p = confirm.resolve("agenda", ok=True)
    assert p and p["action"] == "data" and p["op"] == op           # the mutation survives the confirmation
...
def test_confirm_classify_reply_es_en():
    ...
    assert confirm.classify_reply("¿qué tiempo hace?") is None     # not a yes/no response
    # ambiguous "no, mejor sí" → the last one mentioned wins
    assert confirm.classify_reply("no mejor si") == "yes"


# ── lifecycle.delete_widget (integration: creates and deletes a temporary widget folder) ────────────────
def test_delete_widget_removes_folder_and_tombstones(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert not os.path.isdir(folder)                           # folder deleted from disk
        assert ("delete", wid) in emitted                          # closes the card on the canvas
...
# ── code.py: a DELETE request must never CREATE (that was the bug) ──────────────────────────────────────
def test_code_delete_never_creates(monkeypatch):
    ...
    async def _fake_delete(widget_id, src="system"):                                   # V2-039: provenance
...
# ── FlashBrain: the deletion tools are offered ───────────────────────────────────────────────────────────
