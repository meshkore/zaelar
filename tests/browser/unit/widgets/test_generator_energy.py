...
def test_run_agent_reports_worker_usage_from_json_output(monkeypatch):
    ...
    with patch("nucleo.energy_meter.report_worker_usage") as report:
        ok, err = generator._run_agent("build me a widget")
    assert ok is True
    assert err == ""
...
def test_run_agent_survives_non_json_stdout(monkeypatch):
...
def test_run_agent_skips_energy_report_without_usage_field(monkeypatch):
    monkeypatch.setattr(generator, "_find_claude", lambda: "/usr/bin/claude")
    stdout = '{"result": "done"}'
    monkeypatch.setattr(generator.subprocess, "Popen", lambda *a, **kw: _FakeProc(stdout))
    with patch("nucleo.energy_meter.report_worker_usage") as report:
