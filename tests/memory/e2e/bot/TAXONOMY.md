...
| **D** dedup | ~35 | ~~5+ fraseos~~ ✓B52 (cazó T175); ~~supersede en cadena A→B→C~~ ✓B52 (cazó T175); ~~near-dup que NO es dup~~ ✓B140 (hermano/primo Pedro, 2 citas, 2 tallas: NO se sobre-funden). PENDIENTE: fraseos en varios idiomas. **Bloqueado por T175** (slot=None) para el objetivo dedup ≤1. **Hueco distinto encontrado en auditoría manual 2026-08-16 (V2-103), NO cubierto por ningún caso de este corpus**: la MISMA frase literal (no un fraseo) dicha en DOS turnos reales segundos aparte — todos los casos de esta dimensión son parafraseos, ninguno manda el texto idéntico dos veces. Es justo la forma del bug real (dos hechos "Su suegro se llama Pedro." duplicados, `valid=1` ambos, 3s de diferencia). Cerrado con un test determinista y rápido (`tests/memory/integration/test_realistic_session.py::test_exact_repeat_across_two_real_turns_deduplicates`, vía `ingest_utterance()` real, LLM mockeado) en vez de un caso nuevo aquí — añadirlo a este corpus exigiría un LLM real por cada corrida (coste, no-determinismo) para probar algo que no depende de la calidad de la destilación, así que queda anotado como mejora futura, no bloqueante. |
