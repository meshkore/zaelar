...
# Idempotent + detached; safe to run repeatedly. Logs to tester/runs/guard.log.
set -uo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG="$HERE/tester/runs/guard.log"
...
if ! nc -z 127.0.0.1 43917 2>/dev/null; then
  ...
  ( cd "$HERE" && nohup make run >> "$HERE/tester/runs/zaelar.log" 2>&1 & )
...
# 2) overnight loop
if ! pgrep -f "tester/overnight.sh" >/dev/null 2>&1; then
  echo "[$(ts)] overnight loop down → starting (detached)" >> "$LOG"
  ( cd "$HERE" && nohup bash tester/overnight.sh >> "$HERE/tester/runs/overnight.log" 2>&1 & )
else
