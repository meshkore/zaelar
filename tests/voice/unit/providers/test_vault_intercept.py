...
def _run(text, first_turn=False):
    ...
    handled = asyncio.run(try_vault_intercept(text, first_turn, send, emit))
    return handled, sent, events


def test_plain_chat_is_not_intercepted():
    handled, sent, events = _run("hola, que tal el dia")
...
def test_first_turn_never_intercepted(monkeypatch):
    ...
    handled, sent, events = _run("no me digas los secretos por voz", first_turn=True)
    assert handled is False
    assert not sent and not events
...
def test_security_config_command_is_applied_and_spoken(monkeypatch):
    ...
    handled, sent, events = _run("no me digas los secretos por voz")
...
def test_spoken_secret_without_vault_asks_to_create_one(monkeypatch):
    ...
    handled, sent, events = _run("mi contraseña de Netflix es hunter2")
    assert handled is True
...
def test_a_broken_vault_rules_module_fails_open_to_no_intercept(monkeypatch):
    ...
    handled, sent, events = _run("cualquier cosa")
    assert handled is False
    assert not sent and not events
