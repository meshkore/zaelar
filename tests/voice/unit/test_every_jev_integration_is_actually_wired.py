...
WHY THIS FILE EXISTS (V2-726 §7.1). The five Jev tasks each shipped a test file and all five are
green. One of them — `T-jev-router` — has never made a single call in its life: `ask_route_async`
calls `jev.ask_async(QUESTION_ID, INSTRUCTIONS, KINDS, text)` with four positional arguments into a
one-positional signature, and its own `except Exception: return None` eats the TypeError.

`test_jev_route.py` could not fail on that. Every case builds the handle BY HAND —
`resolve_route(_handle("show"))` — so it exercises the mapping and never the wiring. The one case
that touches the real entry point asserts:

    monkeypatch.setenv("ZAELAR_JEV", "0")
    assert tsel.ask_route_async("hola, ¿qué tal?") is None

…which is exactly what the bug returns. The same `None` means "switched off" and "broken", and
nothing in the suite can tell them apart.
...
@pytest.mark.xfail(strict=True, reason=(
    "V2-726 §3.1 — T-jev-router is born dead: ask_route_async passes four positional args to "
    "ask_async(text, *, run, name) and its own except swallows the TypeError. ZERO calls in 312 "
    "real ones. Left recorded here instead of silently fixed because the repair belongs to V2-726 "
    "F1, which folds this question into the turn brief rather than opening a third socket. "
    "strict=True on purpose: the day F1 lands, this XPASSes and FAILS, forcing the marker out."))
def test_the_route_pre_choice_reaches_the_transport(wire):
    from nucleo.flash import tool_selection as tsel
    _settle(tsel.ask_route_async("abre mis mensajes"))
    assert len(wire.calls) == 1, "the route pre-choice never asked Jev anything"


def test_the_route_is_the_only_integration_that_is_dead(wire):
    """The debt is exactly one. A second silent integration must not be able to hide beside it."""
    ...
    for name, fn in (("filler", lambda: _settle(jev.request_async("open my messages"))),
                     ...
                     ("route", lambda: _settle(tsel.ask_route_async("abre mis mensajes"))),
    ...
    assert fired == ["filler", "canvas", "action", "escalate"], (
        f"the set of LIVE Jev integrations changed: {fired}. If route came alive, retire its "
        f"xfail above; if another one went silent, that is a new V2-726 §3.1.")
