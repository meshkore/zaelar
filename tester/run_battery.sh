#!/usr/bin/env bash
# run_battery.sh — corre TODO el catálogo de escenarios (tester/scenarios.py) contra un zaelar VIVO, uno por uno,
...
for S in "${SCENARIOS[@]}"; do
  ...
  "$PY" -m tester.run --scenario "$S" --no-open --hold 0 >> "$RUNS/battery.log" 2>&1 &
  RUN_PID=$!
...
# Archivo del día (histórico consultable): tester/reports/<YYYYMMDD>-<desc>/ — ver zaelar-testing.md §archiva.
DESC=${BATTERY_DESC:-bateria}
ARCH="$HERE/tester/reports/$(date +%Y%m%d)-$DESC"
mkdir -p "$ARCH"
