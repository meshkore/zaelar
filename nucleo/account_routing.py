...
# token's lifetime (30 days) — avoids a control-plane round-trip on every request.
...
async def find_machine_for_session(
    ...
    (POST /session/verify). Cached + single-flighted. Fail-open: any error (network, no session,
    control-plane not configured) returns None — routing degrades to "serve locally", never a 500."""
