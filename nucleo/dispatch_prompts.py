...
_METHOD_BLOCK = (
    ...
    "candidatos…) o UNA SOLA cosa (la ficha técnica de un producto, un informe, una foto, un resumen) — las dos van "
...
def _build_prompt(request: str, context: str, trusted: bool, brief: dict | None = None) -> str:
    header = ("Eres un Brain Worker del asistente personal zaelar: una sesión de trabajo que CONDUCE una tarea del "
              ...
              "'python -m nucleo.worker_bridge act <accion> <json>'. Si lo que hace falta es que zaelar AVISE "
              "MÁS TARDE (un recordatorio, un seguimiento), prográmalo de verdad con "
              "'python -m nucleo.worker_bridge act schedule {\"when\":\"<cuándo>\",\"prompt\":\"<qué debe "
              "decir o hacer>\"}' — usa «mañana a las 9», «el miércoles a las 18:00», «every 30m» o un "
