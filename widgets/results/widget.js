...
//   { title, subtitle, columns: 1|2, items: [ { title, subtitle, lines:[...], price, badge, url, primary } ] }
...
function makeCard(it, isPrimary){
...
export function render(el, data, ctx){
  ...
  if(primary.length){
    ...
    primary.forEach(it => pgrid.appendChild(makeCard(it, true)));
  }
  ...
  if(rest.length){
    ...
    rest.forEach(it => sgrid.appendChild(makeCard(it, false)));
  }
}
